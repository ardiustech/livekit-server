...
func (pc *PeerConnection) SetLocalDescription(desc SessionDescription) error {
	...
	if weAnswer && remoteDesc != nil {
		...
		fmt.Printf("ARDIUS-DIAG %s SetLocalDescription(answer): configureRTPReceivers done, ENQUEUEING startRTP\n", time.Now().Format(time.RFC3339Nano))
		enqueuedAt := time.Now()
		pc.ops.Enqueue(func() {
			delay := time.Since(enqueuedAt)
			fmt.Printf("ARDIUS-DIAG %s startRTP EXECUTING NOW (queued for %s)\n", time.Now().Format(time.RFC3339Nano), delay)
			pc.startRTP(haveLocalDescription, remoteDesc, currentTransceivers)
			fmt.Printf("ARDIUS-DIAG %s startRTP RETURNED\n", time.Now().Format(time.RFC3339Nano))
		})
	}
	fmt.Printf("ARDIUS-DIAG %s SetLocalDescription RETURNING TO CALLER (weAnswer=%v)\n", time.Now().Format(time.RFC3339Nano), weAnswer)
...
func (pc *PeerConnection) configureRTPReceivers(
	...
	if isRenegotiation { //nolint:nestif
		for _, transceiver := range currentTransceivers {
			...
			var stoppingTrackReason string
			for _, trackRemote := range tracks {
				func(track *TrackRemote) {
					...
					stoppingTrackReason = fmt.Sprintf("mid=%s rid=%q ssrc=%d id=%s streamID=%s found_no_match_in_incoming", mid, track.rid, track.ssrc, track.id, track.streamID)
				}(trackRemote)
			}

			fmt.Printf("ARDIUS-DIAG %s configureRTPReceivers: transceiver mid=%s oldReceiver=%p tracks=%d receiverNeedsStopped=%v reason=%q\n",
				time.Now().Format(time.RFC3339Nano), mid, receiver, len(tracks), receiverNeedsStopped, stoppingTrackReason)
			...
			fmt.Printf("ARDIUS-DIAG %s configureRTPReceivers: REPLACED receiver for mid=%s oldReceiver=%p newReceiver=%p (transceiver.setReceiver about to run)\n",
				time.Now().Format(time.RFC3339Nano), mid, tracks[0], receiver)
			...
			if receiver := t.Receiver(); receiver != nil {
				for _, track := range receiver.Tracks() {
					for _, ssrc := range incomingTrack.ssrcs {
						if ssrc == track.SSRC() {
							fmt.Printf("ARDIUS-DIAG %s configureRTPReceivers: FILTERING OUT incoming ssrc=%d (already claimed by receiver=%p, mid=%s) — treated as already-handled\n",
								time.Now().Format(time.RFC3339Nano), ssrc, receiver, t.Mid())
	...
	fmt.Printf("ARDIUS-DIAG %s configureRTPReceivers: SURVIVING (treated as NEW) tracks after filter: %v\n",
		time.Now().Format(time.RFC3339Nano), diagFmtTrackDetails(filteredTracks))

	for _, incomingTrack := range filteredTracks {
		fmt.Printf("ARDIUS-DIAG %s configureRTPReceivers: routing to configureReceiver (creates a FRESH, not-yet-wired receiver) ssrcs=%v id=%s\n",
			time.Now().Format(time.RFC3339Nano), incomingTrack.ssrcs, incomingTrack.id)
		_ = runIfNewReceiver(incomingTrack, localTransceivers, pc.configureReceiver)
	}
}

// ardiustech DIAGNOSTIC ONLY.
func diagFmtTrackDetails(tracks []trackDetails) string {
	s := "["
